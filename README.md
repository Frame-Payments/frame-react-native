...
| Sonar fraud sessions surviving app restarts | `@react-native-async-storage/async-storage` |
...
> **Without AsyncStorage**, Sonar sessions are held in memory and re-created on each app launch rather than persisting across restarts, matching iOS's `UserDefaults` behavior. Nothing fails; the device simply starts each run with a fresh session.
